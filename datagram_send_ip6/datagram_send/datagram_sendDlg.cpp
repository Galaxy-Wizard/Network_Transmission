...
BOOL Cdatagram_sendDlg::OnInitDialog()
{
	...
	size_t local_size_test = sizeof(sockaddr_in6);
		...
		for(WORD local_counter=0;local_counter<local_parameter_requests_number;local_counter++)
		{
			...
			for(;local_message_length_left_to_send>0;)
			{
				...
				if(domain_name_to_internet_6_name(local_address,local_address_internet_address)==false)
				{
					...
					wcscpy(local_system_error_message,L"domain_name_to_internet_6_name завершилась неудачей");
...
UINT __cdecl datagram_receive_connection_thread(LPVOID parameter)
{
	...
	const int CONST_WAIT_TIME_LISTEN = 100;
	...
	for(;;)
	{
		...
		try
		{
			local_bytes_received = local_blocking_socket.ReceiveDatagram(local_message_to_receive,CONST_MESSAGE_LENGTH,(LPSOCKADDR6)&local_socket_address_peer,CONST_WAIT_TIME_LISTEN);
		}
		catch(CBlockingSocketException *local_blocking_socket_exception)
		{
			const int local_error_message_size = 10000;
