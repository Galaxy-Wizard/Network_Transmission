...
BEGIN_MESSAGE_MAP(Cdatagram_sendDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT_ADDRESS_LOCAL, &Cdatagram_sendDlg::OnEnChangeEditAddressLocal)
	ON_EN_CHANGE(IDC_EDIT_ADDRESS_PORT_NUMBER_LOCAL, &Cdatagram_sendDlg::OnEnChangeEditAddressPortNumberLocal)
	ON_EN_CHANGE(IDC_EDIT_ADDRESS_LOCAL_REVERSE_CONTROL, &Cdatagram_sendDlg::OnEnChangeEditAddressLocalReverseControl)
	ON_EN_CHANGE(IDC_EDIT_ADDRESS_PORT_NUMBER_LOCAL_REVERSE_CONTROL, &Cdatagram_sendDlg::OnEnChangeEditAddressPortNumberLocalReverseControl)
...
BOOL Cdatagram_sendDlg::OnInitDialog()
{
	...
	try
	{
		local_socket.Create(AF_INET6, SOCK_DGRAM);
	}
	...
	if(CONST_MESSAGE_HEADER_LENGTH+CONST_MINIMUM_DATA_LENGTH>UINT(optVal))
	{
		...
		local_format_message.Format(CString(L"Максимум байт доступно для передачи: %d. Байт необходимо для передачи: %d"), UINT(optVal), CONST_MESSAGE_HEADER_LENGTH+CONST_MINIMUM_DATA_LENGTH);
...
UINT __cdecl datagram_send_connection_thread(LPVOID parameter)
{
	...
	for(;local_message_to_send!=NULL && local_main_dialog!=NULL;)
	{
		for(;;)
		{
			...
			try
			{
				local_blocking_socket.Create(AF_INET6, SOCK_DGRAM);
			}
			catch(CBlockingSocketException *local_blocking_socket_exception)
			{
				const int local_error_message_size = 10000;
			...
			if(local_bytes_sent==int(local_message_header.data_size)+int(CONST_MESSAGE_HEADER_LENGTH))
			{
...

				{
					...
					local_string_send_information.Format(CString(L"Байт отправлено: %.0f / %.0f (%2.2f %%)\r\nБайт в секунду (средняя скорость): %.2f\r\nВремя передачи:\r\n %s"),
						double(local_bytes_send_by_all_threads),
						double(local_message_header.file_size),
	...
	for(;local_message_to_receive!=NULL && local_main_dialog!=NULL;)
	{
		...
		for(;;)
		{
			...
			for(;;)
			{
				...
				try
				{
					local_bytes_received = local_blocking_socket.ReceiveDatagram(local_message_to_receive,int(CONST_MESSAGE_LENGTH),(LPSOCKADDR6)&local_socket_address_peer,CONST_WAIT_TIME_LISTEN);
				}
				catch(CBlockingSocketException *local_blocking_socket_exception)
				{
					const int local_error_message_size = 10000;
					...
					if(local_bytes_received==int(local_message_header.data_size)+int(CONST_MESSAGE_HEADER_LENGTH))
					{
						...
						if(local_framents_information_map_iterator!=local_main_dialog->framents_information_receive_map.end())
						{
...

							{
								...
								{
									local_string_receive_information.Format(CString(L"Байт принято: %.0f / %.0f (%2.2f %%)\r\nБайт в секунду  (средняя скорость): %.2f\r\nВремя приёма:\r\n %s"),
										double(local_bytes_received_by_all_threads), double(local_file_size),
...
UINT __cdecl datagram_send_connection_thread_reverse_control(LPVOID parameter)
{
	...
	for(;local_message_to_receive!=NULL && local_main_dialog!=NULL;)
	{
		...
		if(IdnToAscii(0,local_address_local_reverse_control,local_address_local_reverse_control.GetLength(),local_address_unicode,CONST_MESSAGE_LENGTH)==0)
		{
			const int local_error_message_size = 10000;
			wchar_t local_error_message[local_error_message_size];
...
UINT __cdecl datagram_receive_connection_thread_reverse_control(LPVOID parameter)
{
	...
	for(;local_message_to_send!=NULL && local_main_dialog!=NULL;)
	{
		...
		if(IdnToAscii(0,local_address_remote_reverse_control,local_address_remote_reverse_control.GetLength(),local_address_unicode,CONST_MESSAGE_LENGTH)==0)
		{
			const int local_error_message_size = 10000;
			wchar_t local_error_message[local_error_message_size];
...
void Cdatagram_sendDlg::OnEnChangeEditAddressLocal()
{
	...
	CString local_string_text;

	member_edit_address_local.GetWindowTextW(local_string_text);

	member_edit_address_local_external.SetWindowTextW(local_string_text);
}


void Cdatagram_sendDlg::OnEnChangeEditAddressPortNumberLocal()
{
	// TODO:  Если это элемент управления RICHEDIT, то элемент управления не будет
	// send this notification unless you override the CDialogEx::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
	...
	member_edit_port_number_local.GetWindowTextW(local_string_text);

	member_edit_port_number_local_external.SetWindowTextW(local_string_text);
}


void Cdatagram_sendDlg::OnEnChangeEditAddressLocalReverseControl()
{
	// TODO:  Если это элемент управления RICHEDIT, то элемент управления не будет
	// send this notification unless you override the CDialogEx::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
	...
	member_edit_address_local_reverse_control.GetWindowTextW(local_string_text);

	member_edit_address_remote_reverse_control.SetWindowTextW(local_string_text);
}


void Cdatagram_sendDlg::OnEnChangeEditAddressPortNumberLocalReverseControl()
{
	// TODO:  Если это элемент управления RICHEDIT, то элемент управления не будет
	// send this notification unless you override the CDialogEx::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
	...
	member_edit_port_number_local_reverse_control.GetWindowTextW(local_string_text);

	member_edit_port_number_remote_reverse_control.SetWindowTextW(local_string_text);
}
